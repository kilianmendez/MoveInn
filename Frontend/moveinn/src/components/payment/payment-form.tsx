...
function PaymentFormContent() {
  ...
  const fetchAccommodationData = async () => {
    if (!reservation?.accommodationId) {
      console.error(
        "Accommodation ID is undefined. Cannot fetch accommodation data."
      );
    ...
    try {
      const response = await axios.get(
        API_GET_ACCOMMODATION(reservation.accommodationId),
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const deposit = accommodation?.pricePerMonth ?? 0;

    const paymentAmount = totalPrice + deposit;
    ...
    try {
      ...
      if (!createPaymentResponse.ok) {
        const errorResponse = await createPaymentResponse.json();
        console.error("Payment intent creation failed:", errorResponse);
        throw new Error(
          errorResponse.message || "Failed to create payment intent"
        );
      }
      ...
      if (paymentIntent.status === "succeeded") {
        try {
          ...
          if (updateResponse.status !== 200) {
            console.error(
              "Payment successful but failed to update reservation status"
            );
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        <div className="bg-white rounded-xl shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6">
            The accommodation you're looking for doesn't exist or you don't have
            access to it.
          </p>
          <button
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        <div className="bg-gradient-to-br from-white to-green-50 rounded-xl shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6 text-lg">
            Your payment has been processed and your reservation is now
            confirmed.
          </p>
          ...
          <div className="animate-pulse">
            <p className="text-sm text-gray-500">
              Redirecting to your reservation details...
            </p>
          </div>
        </div>
      </div>
  ...
  const total = (monthlyRent ?? 0) + securityDeposit;
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 text-white">
              ...
              <p className="text-blue-100 mt-1">
                Secure payment for your accommodation
              </p>
            </div>
            ...
            <div className="p-6">
              <div className="flex items-start mb-6">
                ...
                <div className="ml-4">
                  ...
                  <div className="flex items-center mt-1 text-sm text-gray-500">
                    <Home className="h-3 w-3 mr-1" />
                    <span>{accommodation.bathrooms} bathroom(s)</span>
                  </div>
                  <div className="flex items-center mt-1 text-sm text-gray-500">
              ...
              <div className="bg-indigo-50 p-4 rounded-lg">
                ...
                <p className="text-xs text-indigo-600 mt-2">
                  By proceeding with the payment, you agree to the terms and
                  conditions of MoveInn.
                </p>
              </div>
            </div>
          </div>

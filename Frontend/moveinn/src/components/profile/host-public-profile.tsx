...
export function HostPublicProfile({ user }: HostPublicProfileProps) {
  const [messageContent, setMessageContent] = useState("")

  // Enhance the user data with host-specific information
  const host = {
    ...user,
    rating: 4.9,
    reviewCount: 42,
    ...
    studyProgram: user.degree || "Economics (3rd year)",
    hometown: user.nationality || "Barcelona, Spain",
    favoriteSpots: [
            ...
            <Card className="border-none shadow-md mb-6 overflow-hidden bg-foreground py-0">
              <div className="bg-gradient-to-r from-[#4C69DD]/20 to-[#62C3BA]/20 p-6 flex flex-col md:flex-row items-center gap-6">
                <div className="relative">
                  <Avatar className="h-32 w-32 border-4 border-white">
                    <AvatarImage src={userAvatar || "/placeholder.svg"} alt={host.name} />
                ...
                <div className="flex-1 text-center md:text-left">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-2 md:gap-4 mb-3">
                    <div className="flex items-center justify-center md:justify-start">
                      ...
                      <span className="text-gray-700">{host.school || "Universitat de Barcelona"}</span>
                    </div>
                    <div className="flex items-center justify-center md:justify-start">
                      <MapPin className="h-4 w-4 text-[#4C69DD] mr-1" />
                      <span className="text-gray-700">{host.city || "Barcelona, Spain"}</span>
                    </div>
                  </div>

                  <div className="flex flex-wrap justify-center md:justify-start gap-2 mb-3">
                    {(host.languages || ["Spanish", "English", "Catalan"]).map((language: string, index: number) => (
                      <Badge
                        key={language}
                        variant="outline"
                        className={`${
              ...
              <CardContent className="p-6">
                ...
                <div className="mb-6">
                  ...
                  <p className="text-gray-700">{host.biography || host.bio}</p>
                </div>

                <div className="mb-6">
                  <h2 className="text-lg font-semibold text-[#0E1E40] mb-2">Areas of Expertise</h2>
                  <div className="flex flex-wrap gap-2">
                    {host.expertise.map((area: string) => (
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {host.interests.map((interest: string) => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <ul className="space-y-2">
                      {host.favoriteSpots.map((spot: string, index: number) => (
